...
from orion_core_afiliados import activar_afiliado, generar_enlace_mejorado
import requests
...
def reportar_venta_tesorero(producto, comision):
    ...
    try:
        requests.post(f"{TESORERO_URL}/reporte", json={
            ...
            "gastos": 0,
            "producto": producto,
            "fecha": os.getenv("DEPLOYMENT_DATE", "2024-01-01")
        }, timeout=5)
...
async def estado(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    estado_sistema += "• Bot de afiliados: 🟢 ACTIVO\n"
    estado_sistema += "• Generación de enlaces: 🟢 FUNCIONANDO\n"
    estado_sistema += "• Tesorero: 🟢 CONECTADO\n" if TESORERO_URL else "• Tesorero: 🟡 NO CONFIGURADO\n"
    estado_sistema += f"• Chat ID: {ADMIN_CHAT_ID}"
...
async def tesorero_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not TESORERO_URL:
        await update.message.reply_text("❌ Tesorero no configurado")
        return
...
def main():
    ...
    print("🚀 Sistema Orion desplegado - Bot de afiliados + Tesorero")
